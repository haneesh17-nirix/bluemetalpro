...
crushersRouter.get('/', async (req, res) => {
  logger.info({ userId: req.user!.id, role: req.user!.role }, 'List crushers');
  try {
    if (req.user!.role === 'admin') {
      const rows = await query('SELECT * FROM crushers WHERE tenant_id = $1 ORDER BY name', [req.user!.tenant_id]);
      logger.info({ userId: req.user!.id, role: req.user!.role, count: (rows as any[]).length }, 'List crushers admin result');
    ...
    logger.info({ userId: req.user!.id, role: req.user!.role, count: (rows as any[]).length }, 'List crushers user result');
    res.json(rows);
  } catch (err) {
    logger.error({ err, userId: req.user!.id, role: req.user!.role }, 'Failed to list crushers');
    return res.status(500).json({ error: 'Failed to list crushers' });
  }
});
...
crushersRouter.post('/', authorize('admin'), async (req, res) => {
  ...
  try {
    logger.debug({ name, gstin, by: req.user!.email }, 'Creating crusher');
    ...
    logAction('crusher.created', { name, gstin, crusherId: (crusher as any).id, by: req.user!.email });
    res.status(201).json(crusher);
  } catch (err) {
    logger.error({ err, name, gstin, by: req.user!.email }, 'Failed to create crusher');
    return res.status(500).json({ error: 'Failed to create crusher' });
  }
});
...
crushersRouter.put('/:id', authorize('admin'), async (req, res) => {
  ...
  try {
    logger.debug({ crusherId: req.params.id, changes: req.body, by: req.user!.email }, 'Updating crusher');
    const crusher = await queryOne(
    ...
    logAction('crusher.updated', { crusherId: req.params.id, changes: req.body, by: req.user!.email });
    res.json(crusher);
  } catch (err) {
    logger.error({ err, crusherId: req.params.id, by: req.user!.email }, 'Failed to update crusher');
    return res.status(500).json({ error: 'Failed to update crusher' });
  }
});
...
crushersRouter.get('/:id/users', authorize('admin'), async (req, res) => {
  try {
    ...
    logger.error({ err, crusherId: req.params.id, by: req.user!.email }, 'Failed to list crusher users');
    return res.status(500).json({ error: 'Failed to list crusher users' });
  }
});
...
crushersRouter.post('/:id/users', authorize('admin'), async (req, res) => {
  ...
  try {
    ...
    logger.error({ err, crusherId: req.params.id, userId: user_id, by: req.user!.email }, 'Failed to grant crusher user access');
    return res.status(500).json({ error: 'Failed to grant crusher user access' });
  }
});
...
crushersRouter.delete('/:id/users/:user_id', authorize('admin'), async (req, res) => {
  try {
    ...
    logger.error({ err, crusherId: req.params.id, targetUserId: req.params.user_id, by: req.user!.email }, 'Failed to revoke crusher user access');
    return res.status(500).json({ error: 'Failed to revoke crusher user access' });
  }
});
