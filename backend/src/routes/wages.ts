...
wagesRouter.get('/workers', async (req, res) => {
  const rows = await query(`SELECT * FROM workers WHERE is_active = true ORDER BY name`);
  res.json(rows);
});
...
wagesRouter.post('/workers', authorize('admin', 'accounts'), async (req, res) => {
  ...
  const w = await queryOne(
    `INSERT INTO workers (name, phone, designation, wage_type, wage_rate, joining_date, aadhaar)
     VALUES ($1,$2,$3,$4,$5,$6,$7) RETURNING *`,
    [name, phone, designation, wage_type || 'daily', wage_rate, joining_date, aadhaar]
  );
...
wagesRouter.put('/workers/:id', authorize('admin', 'accounts'), async (req, res) => {
  ...
  const w = await queryOne(
    ...
    [name, phone, designation, wage_type, wage_rate, req.params.id]
  );
  res.json(w);
});
...
wagesRouter.post('/attendance/bulk', authorize('admin', 'accounts'), async (req, res) => {
  ...
  const results = await Promise.all(entries.map((e: any) =>
    queryOne(
      `INSERT INTO attendance (worker_id, date, status, overtime_hours, advance, created_by)
       VALUES ($1,$2,$3,$4,$5,$6)
       ON CONFLICT (worker_id, date) DO UPDATE SET status=$3, overtime_hours=$4, advance=$5
       RETURNING *`,
      [e.worker_id, date, e.status, e.overtime_hours || 0, e.advance || 0, req.user!.id]
    )
  ));
  res.json(results);
});
...
wagesRouter.get('/attendance', async (req, res) => {
  ...
  const rows = await query(
    `SELECT a.*, w.name as worker_name, w.wage_type, w.wage_rate, w.designation
     FROM attendance a JOIN workers w ON w.id = a.worker_id ${where} ORDER BY a.date DESC, w.name`,
    params
  );
  res.json(rows);
});
